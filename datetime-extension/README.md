...
1. Generates the icon set into `datetime-extension/icons/` (`icon-16.png`, `icon-48.png`, `icon-128.png`).
2. Builds the date/time workspace into `datetime-extension/app/` with relative asset paths (`./assets/...`) so it loads from a `chrome-extension://` or `moz-extension://` origin.

> `datetime-extension/app/` and `datetime-extension/icons/` are generated output and are gitignored — they must be regenerated before loading the extension.
...
4. Click **Load unpacked** and select the `datetime-extension/` folder (the one containing `manifest.json`).
...
3. Click **Load Temporary Add-on…** and select `datetime-extension/manifest.json`.
...
1. Run `bun run build:datetime-extension`.
2. Zip the **contents** of `datetime-extension/` (so `manifest.json` is at the zip root).
...
1. Set your own `browser_specific_settings.gecko.id` in `datetime-extension/manifest.json` (replace `datetime-utilities@yourdomain.com`).
2. Run `bun run build:datetime-extension`.
3. Zip the **contents** of `datetime-extension/`.
