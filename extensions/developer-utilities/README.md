...
1. Generates the icon set into `extensions/developer-utilities/icons/` (`icon-16.png`, `icon-48.png`, `icon-128.png`).
2. Builds the app into `extensions/developer-utilities/app/` with relative asset paths (`./assets/...`) so it loads from a `chrome-extension://` or `moz-extension://` origin.

> `extensions/developer-utilities/app/` and `extensions/developer-utilities/icons/` are generated output and are gitignored — they must be regenerated before loading the extension.
...
4. Click **Load unpacked** and select the `extensions/developer-utilities/` folder (the one containing `manifest.json`).
...
3. Click **Load Temporary Add-on…** and select `extensions/developer-utilities/manifest.json`.
...
1. Run `bun run build:extension`.
2. Zip the **contents** of `extensions/developer-utilities/` (so `manifest.json` is at the zip root).
...
1. Set your own `browser_specific_settings.gecko.id` in `extensions/developer-utilities/manifest.json` (replace `developer-utilities@yourdomain.com`).
2. Run `bun run build:extension`.
3. Zip the **contents** of `extensions/developer-utilities/`.
4. Submit at [addons.mozilla.org](https://addons.mozilla.org/developers/) (free).

## Folder layout

```text
extensions/developer-utilities/
